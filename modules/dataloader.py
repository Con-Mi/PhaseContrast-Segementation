    ...
    def __getitem__(self, index):
        ...
        if self.mode is "train":
            ...
            if self.contour_label_flag and self.watershed_label_flag:
                ...
                contour_label = Image.open(self.contour_label_path)
                watershed_label = Image.open(self.watershed_label_path)
...
def PhaseContrastDataLoader(input_csv_file, label_csv_file, input_chnls=1, data_transform=None, mode="train", batch_sz=2, workers=0):
...
def PhaseContrastTrainValidLoader(input_csv_file, label_csv_file, input_chnls=1, data_transform=None, mode="train", validation_split=0.1, batch_sz=2, workers=0, data_root="../data/AugmentedData"):
    file_idxs = pd.read_csv(input_csv_file)
    label_idxs = pd.read_csv(label_csv_file)
    if data_transform is None:
        data_transform = transforms.ToTensor()
